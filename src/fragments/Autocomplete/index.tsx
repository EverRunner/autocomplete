...
const Autocomplete: React.FC<Props> = ({
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent<HTMLInputElement>) => {
      const shouldGoDown = event.code === KEY_MAP.DOWN && selectIndex < matches.length - 1;
      const shouldGoUp = event.code === KEY_MAP.UP && selectIndex > 0;
      const shouldSelect = event.code === KEY_MAP.ENTER && matches.length > 0;
